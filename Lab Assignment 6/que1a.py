...
def Expand(problem,node):
    ...
    for action in problem.ACTIONS(s):
        ...
        edge_cost = problem.ACTION_COST(s,new)
        child_g = node.g + edge_cost
        child = Node(state=new,parent=node,action=action,g=child_g)
        res.append(child)
...
def A_star(problem):
    frontier=PQ()
    start=Node(state=problem.INITIAL(),parent=None,action=None,g=0)
    ...
    while not frontier.isEmpty():
        node=frontier.pop()
        count+=1
        if node.g > reached[node.state].g:
            continue
